...
map("n", "<leader>e", ":checktime<cr>", { desc = "Reload buffers from disk" })
...
map("n", "<c-h>", '<cmd>lua vim.diagnostic.jump({ count = -1, float = false })<cr>', opts)
map("n", "<c-l>", '<cmd>lua vim.diagnostic.jump({ count = 1, float = false })<cr>', opts)
